  ...
  return (
    <div className="w-full bg-black  h-[fixed]  md:h-[200px] flex flex-col justify-center items-start p-4">
      <div className="grid grid-cols-1    md:grid-cols-3  lg:grid-cols-3 justify-center items-start gap-x-8 gap-y-8  w-full ">
        <p className=" copyright-container-text text-center  md:text-left  text-white font-passion text-base md:text-lg lg:text-xl  transform hover:text-stone-500  ease-in-out duration-700">
        ...
        <div className=" links-container flex flex-row gap-y-5 md:flex-row gap-x-5  justify-center items-center mx-auto  ">
          {links.map((link) => (
            <a
              key={link.label}
              href={link.url}
              className="text-white text-base md:text-lg lg:text-xl  hover:text-stone-500 transform hover:scale-110 ease-in-out duration-700"
            >
              {link.label}
            </a>
          ))}
