    ...
    <div className="w-full flex flex-col gap-y-5 justify-center items-center p-2  mb-[7rem]">
      <section className="w-full grid grid-cols-1    md:grid-cols-2  h-[fixed] justify-center items-center   md:px-[3rem]  p-2">
        {cards_items.map((card, index) => (
          <div key={index} className={`      w-[300px]   md:w-[300px]   lg:w-[500px]   mx-auto  flex flex-col gap-y-5 justify-center items-center p-2  px-5 border border-stone-700   ${
          index ===1?  "relative top-[4rem]" : ""
          }`}>
            <h2 className="text-lg  md:text-xl  lg:text-3xl text-center text-gray-800 ">{card.title}</h2>
            <p className="text-base  md:text-lg  lg:text-xl  text-gray-800 text-center">
              {card.description}
            </p>
          </div>
        ))}
